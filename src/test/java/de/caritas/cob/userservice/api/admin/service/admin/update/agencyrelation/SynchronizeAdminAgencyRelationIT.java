...
public class SynchronizeAdminAgencyRelationIT {
  ...
  @Autowired
  private SynchronizeAdminAgencyRelation synchronizeAdminAgencyRelation;
  @Autowired
  ...
  @Test
  public void synchronizeAdminAgenciesRelation_Should_addCorrectAgenciesForAdmin_When_relationsAreProvided() {
    // given
    long agencyId1 = 90L;
    long agencyId2 = 12L;

    List<CreateAdminAgencyRelationDTO> newAdminAgencyRelationDTOs = createNewAdminAgencyRelationDTOs(
        agencyId1, agencyId2);

    // when
    synchronizeAdminAgencyRelation.synchronizeAdminAgenciesRelation(VALID_ADMIN_ID,
        newAdminAgencyRelationDTOs);

    // then
    List<AdminAgency> result = this.adminAgencyRepository.findByAdminId(VALID_ADMIN_ID);
    assertThat(result, notNullValue());
    assertThat(result, hasSize(2));
    assertThat(result.stream().map(AdminAgency::getAgencyId).collect(Collectors.toList()),
        containsInAnyOrder(agencyId1, agencyId2));
  }

  @Test
  public void synchronizeAdminAgenciesRelation_Should_deleteAgenciesForAdmin_When_relationsAreProvided() {
    // given
    long agencyId2 = 12L;
  ...
  @Test
  public void synchronizeAdminAgenciesRelation_Should_syncAgenciesForAdmin_When_relationsAreProvided() {
    // given
    long agencyId1 = 13L;
    long agencyId2 = 14L;
    long agencyId3 = 15L;
  ...
  private List<CreateAdminAgencyRelationDTO> createNewAdminAgencyRelationDTOs(long... agencyIds) {
    ...
    Arrays.stream(agencyIds).forEach(agencyId -> {
