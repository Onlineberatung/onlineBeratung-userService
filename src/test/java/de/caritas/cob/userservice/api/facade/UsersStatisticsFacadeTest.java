...
class UsersStatisticsFacadeTest {
  ...
  @InjectMocks
  UsersStatisticsFacade usersStatisticsFacade;

  @Test
  void getRegistrationStatistics_Should_ConvertSessionsToResponseDTOAndCallTopicServiceForAllTopicsMap() {
    // given
    Mockito.when(sessionService.findAllSessions()).thenReturn(Lists.newArrayList());

    // when
    RegistrationStatisticsListResponseDTO registrationStatistics = usersStatisticsFacade.getRegistrationStatistics();
