...
public class NewMessageEmailSupplierTest {
  ...
  public void generateEmails_Should_ReturnExpectedMail_When_UserRoleIsUserAndSessionIsTeamSession() {
    ...
    NewMessage newMessage = new NewMessage(teamSession);
    Notifications notifications = new Notifications(newMessage);
    when(settings.getNotifications()).thenReturn(notifications);
    when(consultingTypeManager.getConsultantTypeSettings(any())).thenReturn(settings);
    when(roles.contains(UserRole.USER.getValue())).thenReturn(true);
