...
public class MailDtoBuilderTest {
  ...
  public void build_Should_ReturnCorrectMailDto() {
    ...
    MailDTO result =
        mailDtoBuilder.build(
            EmailNotificationHelper.TEMPLATE_NEW_ENQUIRY_NOTIFICATION,
            EMAIL,
            new SimpleImmutableEntry<String, String>(DATA1_NAME, DATA1_VALUE),
    ...
    assertEquals(
        CORRECT_MAIL_DTO.getTemplateData().get(0).getKey(),
        result.getTemplateData().get(0).getKey());
    assertEquals(
        CORRECT_MAIL_DTO.getTemplateData().get(0).getValue(),
        result.getTemplateData().get(0).getValue());
    assertEquals(
        CORRECT_MAIL_DTO.getTemplateData().get(1).getKey(),
        result.getTemplateData().get(1).getKey());
    assertEquals(
        CORRECT_MAIL_DTO.getTemplateData().get(1).getValue(),
